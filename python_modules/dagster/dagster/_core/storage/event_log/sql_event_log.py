...
class SqlEventLogStorage(EventLogStorage):
            ...
            rows_updated = conn.execute(
                ...
                .where(
                    ...
                    db.and_(
                        ...
                        self._get_asset_check_partition_filter(
    ...
    def _get_asset_check_partition_filter(self, partition_filter: Optional[PartitionKeyFilter]):
        ...
        query = (
            ...
            .where(
                db.and_(
                    ...
                    self._get_asset_check_partition_filter(partition_filter),
                )
            )
    ...
    def get_asset_check_partition_info(
        ...
        infos = []
        # do a bulk fetch of asset records so that we can join in unpartitioned materialization storage ids
        asset_records = self.get_asset_records(list(set(key.asset_key for key in keys)))
